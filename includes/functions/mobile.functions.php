...
function mobile_device_detect($iphone=true,$ipad=true,$android=true,$opera=true,$blackberry=true,$palm=true,$windows=true,$mobileredirect=false,$desktopredirect=false)
{
  ...
  $ret_arr['mobile_browser'] = $mobile_browser;
  $ret_arr['status'] = $status;
  
  return $ret_arr;
}

function func_get_barcode_scan_link($mobile_device)
{
	$scan_link = "";
	if( $mobile_device == 'Apple iPad' || $mobile_device == 'Apple' || $mobile_device == 'Android' )
	{
		$scan_link .= '<a href="http://zxing.appspot.com/scan">Scan a Barcode</a>';
	}
	elseif( $mobile_device == 'Blackberry')
	{
		$scan_link .= '<a href="http://appworld.blackberry.com/webstore/clientlaunch/">Scan a Barcode</a><br />Blackberry users can scan a barocde by going to BB App World and pressing the menu button.';
	}
	else
	{
		$scan_link .= 'Scan a barcode using a Barcode Scanner Application';
	}
	$scan_link .= '<br />Or';
	return $scan_link;
}
